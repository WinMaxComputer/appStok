...
const getters = {
    ...
    SetEditNotaBeli: state => state.editbeli,
...
const actions = {
    ...
    async SimpanPembayaranPembelian({ dispatch }, detail) {
        let response
        try {
            response = await axios.post('/api/store/pembayaran-pembelian', detail)
            const toast = window.Swal.mixin({
                toast: true,
                position: 'top-center',
                showConfirmButton: false,
            ...
            toast.fire({
                icon: 'success',
                title: 'Pembayaran pembelian berhasil disimpan',
                padding: '2em',
            });
            return response;
        } catch (ex) {
            ...
            toast.fire({
                title: 'Error!',
                text: 'Gagal menyimpan pembayaran pembelian',
                icon: 'error',
                padding: '2em'
            });
            throw 'error';
...
const mutations = {
    ...
    SetEditNotaBeli(state, editbeli){
        state.editbeli = editbeli
    },
