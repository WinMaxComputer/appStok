...
var init_runtime_core_esm_bundler = __esmMin((() => {
	...
	hydrateOnIdle = (timeout = 1e4) => (hydrate) => {
		const id = requestIdleCallback(hydrate, { timeout });
		return () => cancelIdleCallback(id);
	};
	hydrateOnVisible = (opts) => (hydrate, forEach) => {
	...
	hydrateOnInteraction = (interactions = []) => (hydrate, forEach) => {
...
var createElement = (tag, attrs, document) => {
	...
	for (const key of Object.keys(attrs)) {
		let value = attrs[key];
		if (key === "key" || value === false) continue;
		if (key === "children") el.textContent = value;
...
var headObjToTags = (obj) => {
	...
	for (const key of Object.keys(obj)) {
		...
		if (key === "title") tags.push({
			tag: key,
			props: { children: obj[key] }
		});
		else if (key === "base") tags.push({
			tag: key,
			props: __spreadValues({ key: "default" }, obj[key])
		});
...
var setAttrs = (el, attrs) => {
	...
	if (existingAttrs) {
		for (const key of existingAttrs.split(",")) if (!(key in attrs)) el.removeAttribute(key);
	}
	const keys = [];
	...
	if (keys.length) el.setAttribute(HEAD_ATTRS_KEY, keys.join(","));
	else el.removeAttribute(HEAD_ATTRS_KEY);
};
var updateElements = (document = window.document, type, tags) => {
	...
	const oldElements = [];
	if (headCountEl) {
		for (let i = 0, j = headCountEl.previousElementSibling; i < headCount; i++, j = (j == null ? void 0 : j.previousElementSibling) || null) if (((_a = j == null ? void 0 : j.tagName) == null ? void 0 : _a.toLowerCase()) === type) oldElements.push(j);
	} else {
	...
	const head = {
		...
		get headTags() {
			...
			allHeadObjs.forEach((objs) => {
				headObjToTags(objs.value).forEach((tag) => {
		...
		updateDOM(document = window.document) {
			...
			for (const tag of tags) updateElements(document, tag, actualTags[tag] || []);
			previousTags.clear();
...
//#endregion
//#region resources/js/src/composables/use-meta.js
var useMeta = (data) => {
	return useHead({
		...data,
		title: `${data.title} | Swamerta`
	});
};
//#endregion
export { withScopeId as $, h as A, normalizeStyle as At, openBlock as B, createSlots as C, isSVGTag as Ct, defineComponent as D, makeMap as Dt, createVNode as E, isVoidTag as Et, onBeforeMount as F, renderSlot as G, provide as H, onBeforeUnmount as I, resolveDynamicComponent as J, resolveComponent as K, onMounted as L, inject as M, shared_esm_bundler_exports as Mt, mergeProps as N, toDisplayString as Nt, getCurrentInstance as O, normalizeClass as Ot, nextTick as P, toHandlerKey as Pt, withDirectives as Q, onUnmounted as R, createElementBlock as S, isReservedProp as St, createTextVNode as T, isSymbol as Tt, pushScopeId as U, popScopeId as V, renderList as W, watch as X, toHandlers as Y, withCtx as Z, Text as _, isBuiltInDirective as _t, init_runtime_dom_esm_bundler as a, shallowReactive as at, createBlock as b, isObject as bt, vModelDynamic as c, EMPTY_OBJ as ct, vModelText as d, camelize as dt, effectScope as et, vShow as f, capitalize as ft, Teleport as g, isArray as gt, Fragment as h, init_shared_esm_bundler as ht, createApp as i, ref as it, init_runtime_core_esm_bundler as j, parseStringStyle as jt, guardReactiveProps as k, normalizeProps as kt, vModelRadio as l, NO as lt, withModifiers as m, generateCodeFrame as mt, createHead as n, isRef as nt, runtime_dom_esm_bundler_exports as o, shallowRef as ot, withKeys as p, extend as pt, resolveDirective as q, Transition as r, reactive as rt, vModelCheckbox as s, unref as st, useMeta as t, init_reactivity_esm_bundler as tt, vModelSelect as u, NOOP as ut, computed as v, isHTMLTag as vt, createStaticVNode as w, isString as wt, createCommentVNode as x, isOn as xt, createBaseVNode as y, isMathMLTag as yt, onUpdated as z };
